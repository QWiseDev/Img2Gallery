...
func scanImage(rows scanner) (map[string]any, error) {
	...
	err := rows.Scan(&id, &userID, &prompt, &imagePath, &taskType, &sourcePath, &isHidden, &status, &errText, &requestIP, &provider, &model, &queued, &started, &completed, &createdAt, &username, &displayName, &avatarColor, &likes, &favorites, &liked, &favorited)
	if err != nil {
		return nil, err
	}
	imageURL := any(nil)
